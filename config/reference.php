 ...
 *     },
 * }
 * @psalm-type ContentfulConfig = array{
 *     delivery?: array<string, array{ // Default: []
 *         default?: bool, // Which client to configure as default for autowiring // Default: false
 *         token: scalar|null, // The Contentful access token for the given space
 *         space: scalar|null, // The space ID
 *         environment?: scalar|null, // The environment ID // Default: "master"
 *         api?: "delivery"|"preview", // The name of the API to use, either "delivery" (default value) or "preview" // Default: "delivery"
 *         options?: array{
 *             locale?: scalar|null, // If set, it will be used as the locale on all API calls // Default: null
 *             host?: scalar|null, // A URL of a host to use instead of cdn.contentful.com (useful with proxies) // Default: null
 *             logger?: scalar|null, // A PSR-3 logger implementation, will default to the system logger // Default: "Psr\\Log\\LoggerInterface"
 *             client?: scalar|null, // A Guzzle client instance // Default: null
 *             cache?: array{
 *                 pool?: scalar|null, // A PSR-6 cache item pool implementation, will default to the system cache // Default: "Psr\\Cache\\CacheItemPoolInterface"
 *                 runtime?: bool, // If true, content type and locale data will be cached during runtime and not on warmup // Default: false
 *                 content?: bool, // If true, entry and asset data will be cached during runtime // Default: false
 *             },
 *             query_cache?: array{
 *                 pool?: scalar|null, // A PSR-6 cache item pool implementation, will default to the system cache. This will be used to cache items retrieved with queries when calling `$client->getEntries($query)` // Default: null
 *                 lifetime?: int, // The number of seconds of lifetime for `$client->getEntries($query)` cache items. There's no invalidation mechanism on these cache items so consider to set a low lifetime (for example 60 seconds). // Default: 60
 *             },
 *         },
 *     }>,
 * }
 ...
 *     },
 * }
 * @psalm-type NetgenLayoutsContentfulConfig = array{
 *     entry_slug_type?: array{
 *         default?: string, // Default: "simple"
 ...
 *     netgen_layouts_contentful?: NetgenLayoutsContentfulConfig,
 *     "when@dev"?: array{
 *         imports?: ImportsConfig,
 *         parameters?: ParametersConfig,
 *         services?: ServicesConfig,
